...
import { aggregateMessageKeysNotFromMe, assertMediaContent, bindWaitForEvent, decryptMediaRetryData, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, encryptSignalProto, extractDeviceJids, generateMessageIDV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, normalizeMessageContent, parseAndInjectE2ESessions, unixTimestampSeconds } from '../Utils/index.js';
import { getUrlInfo } from '../Utils/link-preview.js';
import { areJidsSameUser, getBinaryNodeChild, getBinaryNodeChildren, isJidGroup, isJidUser, isLidUser, jidDecode, jidEncode, jidNormalizedUser, S_WHATSAPP_NET } from '../WABinary/index.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        for (let jid of jids) {
            ...
            if (userDevicesCache.get(user) && useCache) {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isRetryResend) {
                ...
                const participantFulLid = jidEncode(participantLidUser, "lid", participantDevice);
                ...
                const encodedMessageToSend = isMe
                    ...
                    : encodedMsg;
                const { type, ciphertext: encryptedContent } = await encryptSignalProto(participantFulLid, encodedMessageToSend, authState);
                binaryNodeContent.push({
                    tag: "enc",
                    attrs: {
                        v: "2",
